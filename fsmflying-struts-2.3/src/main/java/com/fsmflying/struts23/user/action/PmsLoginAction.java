package com.fsmflying.struts23.user.action;
...
import com.fsmflying.struts23.common.web.constant.SessionConstant;
import com.fsmflying.struts23.user.base.BaseAction;
import com.fsmflying.struts23.user.biz.PmsUserBiz;
import com.fsmflying.struts23.user.entity.PmsUser;
import com.fsmflying.struts23.user.enums.UserStatusEnum;
import com.fsmflying.struts23.user.enums.UserTypeEnum;
...
//	@Autowired
	...
	public String loginPage() {
		if (this.getSessionMap().get(SessionConstant.USER_SESSION_KEY) != null) {
			PmsUser user = (PmsUser) this.getSessionMap().get(SessionConstant.USER_SESSION_KEY);
			if (user != null) {
				this.putData("userNo", user.getUserNo());
		...
		try {
			...
			if (user.getUserPwd().equals(DigestUtils.sha1Hex(pwd))) {
				...
				if (user.getPwdErrorCount().intValue() >= SessionConstant.WEB_PWD_INPUT_ERROR_LIMIT) {
					...
					msg += "<br/>密码错误，再输错【"
							+ (SessionConstant.WEB_PWD_INPUT_ERROR_LIMIT - user.getPwdErrorCount().intValue())
							+ "】次将冻结帐号";
				}
				pmsUserBiz.update(user);
